...
And it will use this information to work out if the year is a leap year and decide the number of days 
in the month, then return that as the output, e.g.:
...
def is_leap(year):
    '''Take a year at parameter 
    to check if a year is a leap year and return True or False
    '''
    if year % 4 == 0:
        if year % 100 == 0:
            if year % 400 == 0:
...
def days_in_month(year, month):
    '''Take a year and month and check if it is a leap year will return 29
    and if not a leap year will return other day in a month'''
